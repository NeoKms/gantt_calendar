    ...
    <div class="d-flex flex-row gantt-container">
        <v-container class="d-flex flex-column autowidth flex-grow-1 dop-cont">
            <div class="d-flex">
                ...
                <v-spacer/>
                <v-btn class="mx-2" fab dark small v-tooltip.auto="`Отменить`" @click="changeState(0)" :disabled="states.length===1 || nowStateInd===0">
                    <v-icon dark>mdi-undo</v-icon>
                </v-btn>
                <v-btn class="mx-2" fab dark small v-tooltip.auto="`Вернуть`" @click="changeState(1)" :disabled="states.length===1||nowStateInd===states.length-1">
                    <v-icon dark>mdi-redo</v-icon>
                </v-btn>
                <v-spacer/>
                <v-btn class="mx-2" fab dark small color="error" v-tooltip.auto="`${step}д. влево`" @click="changeHeader(0)">
                    <v-icon dark>mdi-chevron-left</v-icon>
                </v-btn>
            </div>
            <div v-if="usersWithoutGantt.length">
                <h3> Необходимо настроить график:</h3>
                <p v-for="user in usersWithoutGantt" :key="user.id" class="user-wt-gantt">
                    {{user.fio}}
                    <v-btn class="mx-2" fab dark small icon color="error" v-tooltip.auto="'Создать график'" @click="openEditorGantt(user)">
        ...
        <v-container class="d-flex flex-column justify-start overflow-auto calendar autowidth">
            <div class="d-flex flex-row">
                <div :style="{'max-width': `${columns[0]}px`,'min-width': `${columns[0]}px`}">День</div>
                <div v-for="day in header"  class="dayColumn border-right" :class="day.class" :key="day.s" :ref="`el_${day.i}`" :style="{'max-width': `${timeWidth*24}px`,'min-width': `${timeWidth*24}px`}">{{day.s}}</div>
            </div>
            <div class="d-flex flex-row userrow" v-for="user in usersWithGantt" :key="`${user.id}_${user.upd_timestamp}`" :id="`${user.id}_${user.upd_timestamp}`">
                <div :style="{'max-width': `${columns[0]}px`,'min-width': `${columns[0]}px`}">
                    {{user.fio}}
                    <div class="titleButton" @click="openMenu(user.id)" v-tooltip.auto="'Действия'">
                        <v-icon>{{user.showMenu?'mdi-close':'mdi-dots-vertical'}}</v-icon>
                    </div>
                    <div v-if="user.showMenu" class="titleButton">
                        <ul>
                            <li @click="openModalPeriod(user)">Добавить интервал</li>
                            <li @click="openModalChangeGantt(user)">Изменить график работы</li>
                        </ul>
                    </div>
                </div>
                <template v-for="day in header">
                    <template v-for="(i2,hour) in Array(24)">
                        ...
                        <div v-if="user.ganttData[`${day.i}_${hour}`].menu" :key="`${day.i}_${hour}_menu`" class="titleButton">
                            <ul>
                                <li @click="openModalChange(user.ganttData[`${day.i}_${hour}`],`${day.i}_${hour}`)">Изменить тип интервала</li>
                                <li @click="deleteInterval(user.ganttData[`${day.i}_${hour}`])">Удалить интервал</li>
                            </ul>
                        </div>
                    </template>
                </template>
        ...
        <v-container class="d-flex flex-column autowidth dop-cont">
            <v-btn class="mx-2" fab dark small color="error" v-tooltip.auto="`${step}д. вправо`" @click="changeHeader(1)">
        ...
        <v-dialog width="400" v-model="gantEditor">
            <mGantEditor :uid="dataGantEditor.uidGantEditor" :startDateWork="dataGantEditor.startDateWork" :key="dataGantEditor.uidGantEditor" @close="gantEditor=false" @save="saveGantt"/>
        </v-dialog>
        ...
        <v-dialog width="420" v-model="modalChange">
            <modalChangeType :key="dataModalChange.uid" @close="closeModalChange" :selectedType="dataModalChange.type" :intervals="intervals" @save="saveModalChange"/>
        </v-dialog>
    ...
    export default {
        ...
        data() {
            return {
                ...
                typeNames: {
                    'work': 'Рабочие часы',
                    'daily_duty':'Дежурство',
                    'default': '',
                    'time_off':'Отгул',
                    'vacation':'Отпуск',
                    'at_my_own':'За свой счет',
                    'transfer':'Перенос смены',
                    'sick':'Больничный',
                },
            }
        },
        computed: {
            periodTypes() {
                ...
                for (let type in this.typeColors) {
                    if (this.typeColors)
                        tmp.push({
                            name: type === 'default' ? 'Не рабочее время' : this.typeNames[type],
            ...
            header() {
                ...
                for (let i=this.startDate; i<this.today; i+=86400) {
                    ...
                    arr.push({
                        i: i,
                        s: `${formatDateJS(i, 'DD MMMM YYYY')} (${this.dayType[date.getDay()]})`,
                        d: date.getDate(),
                        y: date.getFullYear(),
                        m: date.getMonth() + 1,
        ...
        methods: {
            ...
            mouseover(evt,item) {
                ...
                if (this.dataModalPeriod.value===item.type) {
                    ...
                    this.dragEnter({preventDefault:()=>{}},item)
            ...
            mouseout(evt,item) {
                ...
                if (this.dataModalPeriod.mouseDown) {
                    this.dragLeave({preventDefault:()=>{}},item)
